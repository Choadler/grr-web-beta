...
function MonthCalendar({ events, month, setMonth }: { events: CombinedScheduleEvent[]; month: string; setMonth: (month: string, preserveScroll?: boolean) => void }) {
  ...
  return <section className="month-calendar" aria-labelledby="calendar-month-title">
    <div className="month-calendar__toolbar"><div className="month-calendar__title"><span>Race calendar</span><h2 id="calendar-month-title" aria-live="polite">{monthLabel(month)}</h2></div><div className="month-calendar__nav"><button className="month-calendar__step" type="button" onClick={() => setMonth(shiftMonth(month, -1), true)} aria-label="Previous month"><span aria-hidden="true">←</span> Previous</button><button type="button" onClick={() => setMonth(currentMonth())}>Today</button><button className="month-calendar__step" type="button" onClick={() => setMonth(shiftMonth(month, 1), true)} aria-label="Next month">Next <span aria-hidden="true">→</span></button></div></div>
    ...
    <div className="month-calendar__grid" role="grid" aria-label={`${monthLabel(month)} race calendar`}>{Array.from({ length: leading }, (_, index) => <div className="month-calendar__blank" aria-hidden="true" key={`blank-${index}`} />)}{days.map((date) => { const dayEvents = byDate.get(date) ?? []; const isToday = date === today; return <section className={`month-calendar__day${dayEvents.length ? ' has-events' : ''}${isToday ? ' is-today' : ''}`} role="gridcell" aria-label={`${Number(date.slice(-2))}, ${dayEvents.length} race${dayEvents.length === 1 ? '' : 's'}`} key={date}><time dateTime={date} aria-current={isToday ? 'date' : undefined}>{Number(date.slice(-2))}</time><div>{dayEvents.map((event) => <CalendarEvent event={event} key={event.id} />)}</div></section> })}</div>
...
export function SchedulePage() {
  ...
  const update = (key: string, value: string, defaultValue: string, preserveScroll = false) => setParams(updateScheduleParams(params, key, value, defaultValue), preserveScroll ? { state: { preserveScroll: true } } : undefined)
  ...
  return <><header className="page-hero page-hero--compact schedule-hero"><div className="container"><p className="eyebrow">Every GRR series · One calendar</p><h1>Race Schedule</h1><p>Upcoming events and official results from the Cup Series, GT League, and IndyCar.</p></div></header><div className="page-content page-content--tight container schedule-page">
    ...
    {status === 'ready' && display === 'calendar' && <MonthCalendar events={filtered} month={month} setMonth={(value, preserveScroll) => update('month', value, currentMonth(), preserveScroll)} />}
