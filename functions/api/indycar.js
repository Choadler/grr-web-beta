...
export async function onRequestGet({ env, request }) {
  ...
  if (url.searchParams.get('list') === 'seasons' || url.searchParams.get('list') === 'schedule-seasons') {
    const seasons = await db.prepare("SELECT id,name,status,is_complete AS isComplete FROM indy_seasons WHERE status<>'draft' ORDER BY created_at DESC").all()
    return json({ seasons: seasons.results })
  }
  const requestedSeason = url.searchParams.get('season')
  const season = requestedSeason
    ? await db.prepare("SELECT id,name,status,is_complete AS isComplete,race_time AS raceTime,timezone FROM indy_seasons WHERE id=? AND status<>'draft' LIMIT 1").bind(requestedSeason).first()
    : await db.prepare("SELECT id,name,status,is_complete AS isComplete,race_time AS raceTime,timezone FROM indy_seasons WHERE status='active' LIMIT 1").first()
