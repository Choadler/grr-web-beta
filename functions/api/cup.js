...
async function loadCup({ env, request }) {
  ...
  const raceEvents = events.map((event) => {
    ...
    eventRows.filter((row) => row.session_type === 'SEGMENT').forEach((row) => {
      stageTotals.set(row.srh_driver_id, (stageTotals.get(row.srh_driver_id) ?? 0) + (row.stage_points ?? 0))
    })
    const sessionIds = [...new Set(eventRows.map((row) => row.srh_race_id))].sort((left, right) => {
      const leftRow = eventRows.find((row) => row.srh_race_id === left)
      const rightRow = eventRows.find((row) => row.srh_race_id === right)
      return (leftRow?.session_type === 'RACE' ? 0 : 1) - (rightRow?.session_type === 'RACE' ? 0 : 1) || (leftRow?.sort_order ?? 0) - (rightRow?.sort_order ?? 0)
    })
    let stage = 0
